...
Example
-------
    from aat_system.connect import ask
...
Source: TRITONAI_SETUP.md Part 3, copied verbatim except for ``DEFAULT_MODEL``
(set from the setup interview) and the ``oauth_gpt`` import, which follows this
module's package placement rather than a top-level ``utils/``.
"""
...
# OAuth path in ``aat_system.oauth_gpt`` instead. Set ``MODEL = "oauth-gpt"`` to
# use your personal ChatGPT/Codex subscription via OAuth (no API key needed).
...
def ask(
    ...
    **No fallbacks.** If ``model`` is unknown, unauthorized, or the endpoint
    ...
    credentials loading (see ``aat_system.oauth_gpt.ensure_openai_codex_credentials``):
